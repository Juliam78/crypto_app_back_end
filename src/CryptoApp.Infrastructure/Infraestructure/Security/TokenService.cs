...
namespace CryptoAppBackEnd.Infraestructure.Security
{
    ...
    /// base64url). Lo emite y valida únicamente este backend. Permite restaurar la sesión tras
    /// refrescar la página sin volver a pedir credenciales.
    /// Idéntico al adaptador del microservicio AuthService.
    ...
    public class TokenService : ITokenService
    {
        ...
        {
            var role = person.role == 'A' ? "admin" : "user";
            return Create(person.id, person.email, role);
        }
